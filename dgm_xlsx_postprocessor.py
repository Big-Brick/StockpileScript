...
RANK_ALIAS_PATTERNS: Sequence[Tuple[str, str]] = (
	...
	(r"ст\.?\s*солд(?:ат)?\.?|старший\s+солдат", "старший солдат"),

	# Рядовий склад — корабельні
	(r"матр(?:ос)?\.?", "матрос"),
	(r"ст\.?\s*матр(?:ос)?\.?|старший\s+матрос", "старший матрос"),

	# Молодший сержантський / старшинський склад
	(r"(?:мол|мл)\.?\s*серж(?:ант)?\.?|мл\.?\s*с-т|м/с-т|молодший\s+сержант", "молодший сержант"),
	(r"серж(?:ант)?\.?|с-нт\.?|с-т", "сержант"),
	...
	(r"ст\.?\s*серж(?:ант)?\.?|ст\.?\s*с-т|старший\s+сержант", "старший сержант"),
	(r"гол\.?\s*серж(?:ант)?\.?|головний\s+сержант", "головний сержант"),
	(r"(?:штаб[ -]|шт\.?\s*)серж(?:ант)?\.?", "штаб-сержант"),
	(r"гол\.?\s*старшина|головний\s+старшина", "головний старшина"),
	(r"гол\.?\s*корабельний\s+старшина|головний\s+корабельний\s+старшина", "головний корабельний старшина"),
	(r"штаб[ -]старшина|шт\.?\s*старшина", "штаб-старшина"),

	# Вищий сержантський / старшинський склад
	(r"майст(?:е|)р[ -]сержант", "майстер-сержант"),
	(r"(?:ст\.?|старший)\s+майстер[ -]сержант", "старший майстер-сержант"),
	(r"(?:гол\.?|головний)\s+майстер[ -]сержант", "головний майстер-сержант"),
	(r"майстер[ -]старшина", "майстер-старшина"),
	(r"(?:ст\.?|старший)\s+майстер[ -]старшина", "старший майстер-старшина"),
	(r"(?:гол\.?|головний)\s+майстер[ -]старшина", "головний майстер-старшина"),

	# Молодший офіцерський склад
	(r"(?:мол|мл)\.?\s*лейтенант|(?:мл\.?\s*|м/)л-нт|молодший\s+лейтенант", "молодший лейтенант"),
	(r"лейтенант|л-нт|лнт|л-т|лейт\.?", "лейтенант"),
	(r"ст\.?\s*лейтенант|ст\.?\s*л-(?:нт|т)|старший\s+лейтенант", "старший лейтенант"),
	(r"капітан|кап\.?|к-н\.?", "капітан"),
	(r"капітан[ -]лейтенант|кап\.?[ -]?лейтенант|кап\.?[-\s]*л-нт", "капітан-лейтенант"),

	# Старший офіцерський склад
	(r"майор|м-р|мр", "майор"),
	(r"підполковник|підп-?к|п/п-?к|пп-?к", "підполковник"),
	(r"полковник|полк\.?|п-к|пк", "полковник"),
	(r"(?:капітан|кап\.?)\s+(?:3|iii)\s+(?:рангу|р\.?)", "капітан 3 рангу"),
	(r"(?:капітан|кап\.?)\s+(?:2|ii)\s+(?:рангу|р\.?)", "капітан 2 рангу"),
	(r"(?:капітан|кап\.?)\s+(?:1|i)\s+(?:рангу|р\.?)", "капітан 1 рангу"),

	# Вищий офіцерський склад
	(r"бриг\.?\s*ген(?:ерал)?\.?|бригадний\s+генерал", "бригадний генерал"),
	(r"генерал[ -]майор|ген\.?-?\s*майор|ген\.?\s*м-р", "генерал-майор"),
	(r"генерал[ -]лейтенант|ген\.?-?\s*лейтенант|ген\.?\s*л-нт", "генерал-лейтенант"),
	(r"генерал|ген\.?", "генерал"),
	(r"комм?одор", "коммодор"),
	(r"контр[ -]адмірал|к\.?-?\s*адмірал|контр-адм\.?", "контр-адмірал"),
	(r"віце[ -]адмірал|в\.?-?\s*адмірал|віце-адм\.?", "віце-адмірал"),
	(r"адмірал|адм\.?", "адмірал"),

	# Старі звання, які вже не присвоюються в цій системі, але можуть траплятись у документах
	(r"генерал[ -]полковник|ген\.?-?\s*полковник", "генерал-полковник"),
	(r"генерал\s+армії(?:\s+україни)?|ген\.?\s+армії\s+україни", "генерал армії україни"),
)
